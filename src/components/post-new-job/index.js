...
function PostNewJob({ profileInfo, user, jobList }) {
  ...
  function handleAddNewJob() {
    ...
    if(!profileInfo?.isPremiumUser && jobList.length >= 7) {
      toast({
        variant: "destructive",
        title: "You can post unlimited jobs.",
        description: "Please opt for membership to post more jobs",
      });
      return;
    }
  ...
  return (
    <div>
      <Button
        ...
        className="flex items-center justify-center px-5 disabled:opacity-60 h-11"
      >
        Post A Job
      </Button>
      <Dialog
